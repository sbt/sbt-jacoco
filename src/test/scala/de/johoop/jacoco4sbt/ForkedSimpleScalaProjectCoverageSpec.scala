...
"""
  ...
  lazy val exitCode = Process(Seq(Util.processName, s"-Dsbt.version=${BuildInfo.sbtVersion}", s"-Dplugin.version=${BuildInfo.version}", "clean", "jacoco:cover"), Some(testProjectBase)) !
